...
export default function Dashboard() {
    ...
    const handleSubscribe = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (data.success) {
                alert("✅ Inscription réussie ! Vous recevrez les alertes majeures.");
    ...
    // Calculate oil price metrics (extracted from JSX for stability)
    const latestOil = React.useMemo(() => {
        return oilPrices && oilPrices.length > 0 ? oilPrices[0] : null;
    }, [oilPrices]);

    const brentCurrent = Number(latestOil?.brent || 81.24);
    const wtiCurrent = Number(latestOil?.wti || 77.30);

    // Get 7-day trend data
    const last7Days = React.useMemo(() => {
        // Safe check for valid data
        if (!oilPrices || !Array.isArray(oilPrices) || oilPrices.length === 0) return [];
        return oilPrices.length >= 7
            ? oilPrices.slice(0, 7).reverse()
            : oilPrices.slice().reverse(); // Use whatever we have if less than 7
    }, [oilPrices]);

    const brentTrend = React.useMemo(() => {
        if (last7Days.length > 0) {
            return last7Days.map(d => Number(d.brent || 0));
        }
        return [79.1, 78.8, 79.5, 80.2, 80.8, 81.0, brentCurrent];
    }, [last7Days, brentCurrent]);

    const wtiTrend = React.useMemo(() => {
        if (last7Days.length > 0) {
            return last7Days.map(d => Number(d.wti || 0));
        }
        return [75.2, 75.0, 75.8, 76.4, 76.9, 77.1, wtiCurrent];
    }, [last7Days, wtiCurrent]);

    // Calculate percentage changes with safety
    const brentChange = React.useMemo(() => {
        if (brentTrend.length >= 2) {
            const first = Number(brentTrend[0]) || 0;
            const last = Number(brentTrend[brentTrend.length - 1]) || 0;
            if (first !== 0) {
    ...
    const wtiChange = React.useMemo(() => {
        if (wtiTrend.length >= 2) {
            const first = Number(wtiTrend[0]) || 0;
            const last = Number(wtiTrend[wtiTrend.length - 1]) || 0;
            if (first !== 0) {
                return (((last - first) / first) * 100).toFixed(1);
            }
        }
    ...
    const brentDailyChange = React.useMemo(() => {
        if (oilPrices && oilPrices.length >= 2) {
            const current = Number(oilPrices[0].brent);
            const previous = Number(oilPrices[1].brent);
            if (current && previous) {
    ...
    const wtiDailyChange = React.useMemo(() => {
        if (oilPrices && oilPrices.length >= 2) {
            const current = Number(oilPrices[0].wti);
            const previous = Number(oilPrices[1].wti);
            if (current && previous) {
                return (((current - previous) / previous) * 100).toFixed(1);
            }
        }
    ...
    const brentDailyDiff = React.useMemo(() => {
        if (oilPrices && oilPrices.length >= 2) {
            const current = Number(oilPrices[0].brent);
            const previous = Number(oilPrices[1].brent);
            if (current && previous) {
            ...
            <header className="h-24 border-b border-[#00d4ff1a] bg-[#0d1526d9] backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <button
                        ...
                        className="ml-2 flex items-center gap-2 px-3 py-1.5 bg-[#00ff88]/10 hover:bg-[#00ff88]/20 border border-[#00ff88]/30 rounded text-[10px] font-mono text-[#00ff88] transition-all"
                    >
                        <span className="font-bold tracking-wider hidden md:inline">S'ABONNER AUX ALERTES</span>
                    </button>
                </div>
